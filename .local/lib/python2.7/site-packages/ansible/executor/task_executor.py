...
def start_connection(play_context, variables):
    ...
    p = subprocess.Popen(
        ...
        stdin=subordinate, stdout=subprocess.PIPE, stderr=subprocess.PIPE, env=env
    )
    ...
    # truncating.
    old = termios.tcgetattr(main)
    new = termios.tcgetattr(main)
    new[3] = new[3] & ~termios.ICANON

    try:
        termios.tcsetattr(main, termios.TCSANOW, new)
        write_to_file_descriptor(main, variables)
        write_to_file_descriptor(main, play_context.serialize())

        (stdout, stderr) = p.communicate()
    finally:
        termios.tcsetattr(main, termios.TCSANOW, old)
    os.close(main)
