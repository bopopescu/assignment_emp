...
DOCUMENTATION = '''
    ...
    options:
      ...
      ssh_args:
          ...
          default: '-C -o ControlMain=auto -o ControlPersist=60s'
          ini:
              - section: 'ssh_connection'
                key: 'ssh_args'
          env:
        ...
        if not in_data:
            try:
                ...
                if PY3 and self._play_context.password:
                    ...
                    p = subprocess.Popen(cmd, stdin=subordinate, stdout=subprocess.PIPE, stderr=subprocess.PIPE, pass_fds=self.sshpass_pipe)
                else:
                    p = subprocess.Popen(cmd, stdin=subordinate, stdout=subprocess.PIPE, stderr=subprocess.PIPE)
                stdin = os.fdopen(main, 'wb', 0)
                ...
                for key, event in events:
                    if key.fileobj == p.stdout:
                        ...
                        if b_chunk == b'':
                            ...
                            # When ssh has ControlMain (+ControlPath/Persist) enabled, the
