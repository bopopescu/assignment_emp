...
DOCUMENTATION = """
    ...
    notes:
      - This lookup will use an existing 1Password session if one exists. If not, and you have already
        performed an initial sign in (meaning C(~/.op/config exists)), then only the C(main_password) is required.
        You may optionally specify C(subdomain) in this scenario, otherwise the last used subdomain will be used by C(op).
      - This lookup can perform an initial login by providing C(subdomain), C(username), C(secret_key), and C(main_password).
      - Due to the B(very) sensitive nature of these credentials, it is B(highly) recommended that you only pass in the minimal credentials
        needed at any given time. Also, store these credentials in an Ansible Vault using a key that is equal to or greater in strength
        to the 1Password main password.
...
- name: Retrieve password for HAL when not signed in to 1Password
  debug:
    var: lookup('onepassword'
                'HAL 9000'
                subdomain='Discovery'
                main_password=vault_main_password)
...
class OnePass(object):
    ...
    def get_token(self):
        ...
        if os.path.isfile(self.config_file_path):
            ...
            if not self.main_password:
                raise AnsibleLookupError('Unable to sign in to 1Password. main_password is required.')
            ...
            try:
                ...
                rc, out, err = self._run(args, command_input=to_bytes(self.main_password))
    ...
    def full_login(self):
        if None in [self.subdomain, self.username, self.secret_key, self.main_password]:
...
class LookupModule(LookupBase):
    ...
    def run(self, terms, variables=None, **kwargs):
        ...
        op.main_password = kwargs.get('main_password', kwargs.get('vault_password'))
