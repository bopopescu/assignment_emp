...
class User(object):
    ...
    def ssh_key_gen(self):
        ...
        if self.ssh_passphrase is not None:
            ...
            main_in_fd, subordinate_in_fd = pty.openpty()
            main_out_fd, subordinate_out_fd = pty.openpty()
            main_err_fd, subordinate_err_fd = pty.openpty()
            env = os.environ.copy()
            env['LC_ALL'] = 'C'
            try:
                p = subprocess.Popen([to_bytes(c) for c in cmd],
                                     stdin=subordinate_in_fd,
                                     stdout=subordinate_out_fd,
                                     stderr=subordinate_err_fd,
                                     preexec_fn=os.setsid,
                                     env=env)
                out_buffer = b''
                err_buffer = b''
                while p.poll() is None:
                    r, w, e = select.select([main_out_fd, main_err_fd], [], [], 1)
                    first_prompt = b'Enter passphrase (empty for no passphrase):'
                    second_prompt = b'Enter same passphrase again'
                    prompt = first_prompt
                    for fd in r:
                        if fd == main_out_fd:
                            chunk = os.read(main_out_fd, 10240)
                            out_buffer += chunk
                            if prompt in out_buffer:
                                os.write(main_in_fd, to_bytes(self.ssh_passphrase, errors='strict') + b'\r')
                                prompt = second_prompt
                        else:
                            chunk = os.read(main_err_fd, 10240)
                            err_buffer += chunk
