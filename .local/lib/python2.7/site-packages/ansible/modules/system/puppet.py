...
options:
  ...
  puppetmain:
    description:
      - The hostname of the puppetmain to contact.
    type: str
  modulepath:
    description:
  ...
  execute:
    description:
      ...
      - It has no effect with a puppetmain.
    type: str
    version_added: "2.1"
  use_srv_records:
    description:
...
- name: Run puppet using a specific piece of Puppet code. Has no effect with a puppetmain
  puppet:
...
def main():
    module = AnsibleModule(
        ...
        mutually_exclusive=[
            ('puppetmain', 'manifest'),
            ('puppetmain', 'manifest', 'execute'),
            ('puppetmain', 'modulepath'),
        ],
    )
    p = module.params
    ...
    if not p['manifest'] and not p['execute']:
        ...
        if p['puppetmain']:
            cmd += " --server %s" % shlex_quote(p['puppetmain'])
        if p['show_diff']:
            cmd += " --show_diff"
        if p['environment']:
