    ...
    auto_login:
        ...
        description:
            ...
            - It is B(highly) recommended to store 1Password credentials in an Ansible Vault. Ensure that the key used to encrypt
              the Ansible Vault is equal to or greater in strength than the 1Password main password.
        suboptions:
            subdomain:
                type: str
                description:
            ...
            main_password:
                type: str
                description:
                    - The main password for your subdomain.
    ...
    def full_login(self):
        if self.auto_login is not None:
            if None in [self.auto_login.get('subdomain'), self.auto_login.get('username'),
                        self.auto_login.get('secret_key'), self.auto_login.get('main_password')]:
                module.fail_json(msg='Unable to perform initial sign in to 1Password. '
                                     'subdomain, username, secret_key, and main_password are required to perform initial sign in.')
            ...
            try:
                rc, out, err = self._run(args, command_input=to_bytes(self.auto_login['main_password']))
    ...
    def get_token(self):
        ...
        if os.path.isfile(self.config_file_path):
            ...
            if self.auto_login is not None:
                ...
                # Since we are not currently signed in, main_password is required at a minimum
                if not self.auto_login.get('main_password'):
                    module.fail_json(msg="Unable to sign in to 1Password. 'auto_login.main_password' is required.")

                # Try signing in using the main_password and a subdomain if one is provided
                try:
    ...
    module = AnsibleModule(
        argument_spec=dict(
            ...
            auto_login=dict(type='dict', options=dict(
                ...
                main_password=dict(required=True, type='str', no_log=True),
