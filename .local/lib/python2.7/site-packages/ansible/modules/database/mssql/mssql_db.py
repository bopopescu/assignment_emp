...
def db_exists(conn, cursor, db):
    cursor.execute("SELECT name FROM main.sys.databases WHERE name = %s", db)
    conn.commit()
...
def main():
    ...
    try:
        conn = pymssql.connect(user=login_user, password=login_password, host=login_querystring, database='main')
        cursor = conn.cursor()
