...
description:
   ...
   - The module allows to reset parameter to boot_val (cluster initial value) by I(reset=yes) or remove parameter
     string from postgresql.auto.conf and reload I(value=default) (for settings with postmain context restart is required).
...
- Pay attention, change setting with 'postmain' context can return changed is true
