...
options:
  filter:
    ...
    - Allowable values are C(version), C(databases), C(settings), C(users),
      C(subordinate_status), C(subordinate_hosts), C(main_status), C(engines).
...
# Display only subordinate status:
# ansible standby -m mysql_info -a 'filter=subordinate_status'
...
main_status:
  description: Main status information.
  returned: if main
  type: dict
  sample:
  - { "Binlog_Do_DB": "", "Binlog_Ignore_DB": "mysql", "File": "mysql-bin.000001", "Position": 769 }
subordinate_status:
  description: Subordinate status information.
  returned: if standby
  type: dict
  sample:
  - { "192.168.1.101": { "3306": { "replication_user": { "Connect_Retry": 60, "Exec_Main_Log_Pos": 769,  "Last_Errno": 0 } } } }
subordinate_hosts:
  description: Subordinate status information.
  returned: if main
  type: dict
  sample:
  - { "2": { "Host": "", "Main_id": 1, "Port": 3306 } }
'''
...
class MySQL_Info(object):
    ...
    def __collect(self):
        ...
        self.__get_engines()
        self.__get_users()
        self.__get_main_status()
        self.__get_subordinate_status()
        self.__get_subordinates()
    ...
    def __get_main_status(self):
        """Get main status if the instance is a main."""
        res = self.__exec_sql('SHOW MASTER STATUS')
        if res:
            for line in res:
                for vname, val in iteritems(line):
                    self.info['main_status'][vname] = self.__convert(val)

    def __get_subordinate_status(self):
        """Get subordinate status if the instance is a subordinate."""
        res = self.__exec_sql('SHOW SLAVE STATUS')
        if res:
            for line in res:
                host = line['Main_Host']
                if host not in self.info['subordinate_status']:
                    self.info['subordinate_status'][host] = {}

                port = line['Main_Port']
                if port not in self.info['subordinate_status'][host]:
                    self.info['subordinate_status'][host][port] = {}

                user = line['Main_User']
                if user not in self.info['subordinate_status'][host][port]:
                    self.info['subordinate_status'][host][port][user] = {}

                for vname, val in iteritems(line):
                    if vname not in ('Main_Host', 'Main_Port', 'Main_User'):
                        self.info['subordinate_status'][host][port][user][vname] = self.__convert(val)

    def __get_subordinates(self):
        """Get subordinate hosts info if the instance is a main."""
        res = self.__exec_sql('SHOW SLAVE HOSTS')
        if res:
            for line in res:
                srv_id = line['Server_id']
                if srv_id not in self.info['subordinate_hosts']:
                    self.info['subordinate_hosts'][srv_id] = {}

                for vname, val in iteritems(line):
                    if vname != 'Server_id':
                        self.info['subordinate_hosts'][srv_id][vname] = self.__convert(val)
