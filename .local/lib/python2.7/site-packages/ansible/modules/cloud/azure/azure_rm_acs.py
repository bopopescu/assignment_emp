...
options:
    ...
    main_profile:
        description:
            - Main profile suboptions.
        required: true
        suboptions:
            count:
                description:
                    - Number of mains (VMs) in the container service cluster. Allowed values are C(1), C(3), and C(5).
                required: true
                choices:
                  - 1
                  - 3
            ...
            dns_prefix:
                description:
                    - The DNS Prefix to use for the Container Service main nodes.
                required: true
    linux_profile:
        description:
...
try:
    ...
    from azure.mgmt.containerservice.models import (
        ...
        ContainerServiceServicePrincipalProfile, ContainerServiceMainProfile,
...
def create_main_profile_instance(mainprofile):
    '''
    Helper method to serialize a dict to a ContainerServiceMainProfile
    ...
    and Azure server.
    :param: mainprofile: dict with the parameters to setup the ContainerServiceMainProfile
    :return: ContainerServiceMainProfile
    '''
    return ContainerServiceMainProfile(
        count=mainprofile[0]['count'],
        dns_prefix=mainprofile[0]['dns_prefix'],
        vm_size=mainprofile[0]['vm_size'],
...
def create_acs_dict(acs):
    ...
    return dict(
        ...
        main_profile=create_main_profile_dict(acs.main_profile),
...
def create_main_profile_dict(mainprofile):
    '''
    Helper method to deserialize a ContainerServiceMainProfile to a dict
    :param: mainprofile: ContainerServiceMainProfile with the Azure callback object
    :return: dict with the state on Azure
    '''
    return dict(
        count=mainprofile.count,
        fqdn=mainprofile.fqdn,
        vm_size=mainprofile.vm_size,
        dns_prefix=mainprofile.dns_prefix
    )
...
class AzureRMContainerService(AzureRMModuleBase):
    ...
    def exec_module(self, **kwargs):
        ...
        if self.state == 'present':
            ...
            maincount = self.main_profile[0].get('count')
            if maincount != 1 and maincount != 3 and maincount != 5:
                self.fail('Main Count number wrong : {0} / should be 1 3 or 5'.format(maincount))
            ...
            if not response:
                ...
                if response['provisioning_state'] == "Succeeded":
                    ...
                    # Cannot Update the main count for now // Uncomment this block in the future to support it
                    if is_property_changed('main_profile', 'count'):
                        # self.log(("Main Profile Count Diff, Was {0} / Now {1}"
                        #           .format(response['main_profile'].count,
                        #           self.main_profile[0].get('count'))))
                        # to_be_updated = True
                        self.module.warn("main_profile.count cannot be updated")

                    # Cannot Update the main vm_size for now. Could be a client SDK bug
                    # Uncomment this block in the future to support it
                    if is_property_changed('main_profile', 'vm_size', True):
                        # self.log(("Main Profile VM Size Diff, Was {0} / Now {1}"
                        #           .format(response['main_profile'].get('vm_size'),
                        #                   self.main_profile[0].get('vm_size'))))
                        # to_be_updated = True
                        self.module.warn("main_profile.vm_size cannot be updated")
    ...
    def create_update_acs(self):
        '''
        Creates or updates a container service with the specified configuration of orchestrator, mains, and agents.
        ...
        parameters = ContainerService(
            ...
            main_profile=create_main_profile_instance(self.main_profile),
        ...
        # self.log("main_profile : {0}".format(parameters.main_profile))
