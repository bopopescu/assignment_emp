...
options:
  ...
  encryption:
    ...
    version_added: "2.9"
  encryption_key_id:
    description: KMS main key ID to use for the default encryption. This parameter is allowed if encryption is aws:kms. If
...
def create_or_update_bucket(s3_client, module, location):
    ...
    if encryption is not None:
        ...
        current_encryption_key = current_encryption.get('KMSMainKeyID') if current_encryption else None
        if encryption == 'none' and current_encryption_algorithm is not None:
            ...
            if encryption == 'aws:kms':
                expected_encryption.update({'KMSMainKeyID': encryption_key_id})
            try:
