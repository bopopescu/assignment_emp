...
description:
    - Manage OpenStack DNS zones. Zones can be created, deleted or
      updated. Only the I(email), I(description), I(ttl) and I(mains) values
      can be updated.
options:
   name:
     description:
   ...
   mains:
     description:
        - Main nameservers (only applies if zone_type is secondary)
   state:
     description:
...
zone:
    ...
    contains:
        ...
        ttl:
            ...
            sample: 3600
        mains:
            description: Zone main nameservers
            type: list
            sample: []
'''
...
def _system_state_change(state, email, description, ttl, mains, zone):
    if state == 'present':
        if not zone:
            return True
        ...
        if ttl is not None and zone.ttl != ttl:
            return True
        if mains is not None and zone.mains != mains:
            return True
...
def main():
    argument_spec = openstack_full_argument_spec(
        ...
        mains=dict(required=False, default=None, type='list'),
        ...
        if state == 'present':
            ...
            if zone is None:
                zone = cloud.create_zone(
                    ...
                    description=description, ttl=ttl, mains=mains)
                changed = True
            else:
                if mains is None:
                    mains = []
