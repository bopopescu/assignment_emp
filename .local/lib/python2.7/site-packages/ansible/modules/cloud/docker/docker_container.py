  ...
  volumes:
    description:
      ...
      - "Mount modes can be a comma-separated list of various modes such as C(ro), C(rw), C(consistent),
        C(delegated), C(cached), C(rprivate), C(private), C(rshared), C(shared), C(rsubordinate), C(subordinate), and
...
- name: Start a container and use an env file
  docker_container:
    name: agent
    image: jenkinsci/ssh-subordinate
...
def is_volume_permissions(mode):
    for part in mode.split(','):
        if part not in ('rw', 'ro', 'z', 'Z', 'consistent', 'delegated', 'cached', 'rprivate', 'private', 'rshared', 'shared', 'rsubordinate', 'subordinate', 'nocopy'):
            return False
    return True
...
def main():
    argument_spec = dict(
        ...
        mounts=dict(type='list', elements='dict', options=dict(
            ...
            propagation=dict(type='str', choices=['private', 'rprivate', 'shared', 'rshared', 'subordinate', 'rsubordinate']),
