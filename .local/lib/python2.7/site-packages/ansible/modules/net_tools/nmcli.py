...
options:
    ...
    type:
        ...
        type: str
        choices: [ bond, bond-subordinate, bridge, bridge-subordinate, ethernet, generic, ipip, sit, team, team-subordinate, vlan, vxlan ]
    mode:
        description:
    ...
    main:
        description:
            - Main <main (ifname, or connection UUID or conn_name) of bridge, team, bond main connection profile.
        type: str
    ip4:
        description:
    ...
    subordinatepriority:
        description:
            - This is only used with 'bridge-subordinate' - [<0-63>] - STP priority of this subordinate.
        type: int
        default: 32
    path_cost:
        description:
            - This is only used with 'bridge-subordinate' - [<1-65535>] - STP port cost for destinations via this subordinate.
        type: int
        default: 100
    hairpin:
        description:
            - This is only used with 'bridge-subordinate' - 'hairpin mode' for the subordinate, which allows frames to be sent back out through the subordinate the
...
##### Working with all cloud nodes - Teaming
  ...
  - name: Try nmcli add teams-subordinate
    nmcli:
      type: team-subordinate
      conn_name: '{{ item.conn_name }}'
      ifname: '{{ item.ifname }}'
      main: '{{ item.main }}'
      state: present
...
###### Working with all cloud nodes - Bonding
  ...
  - name: Try nmcli add bond-subordinate
    nmcli:
      type: bond-subordinate
      conn_name: '{{ item.conn_name }}'
      ifname: '{{ item.ifname }}'
      main: '{{ item.main }}'
      state: present
...
class Nmcli(object):
    ...
    def __init__(self, module):
        ...
        self.subordinatepriority = module.params['subordinatepriority']
    ...
    def create_connection_team_subordinate(self):
        cmd = [self.nmcli_bin, 'connection', 'add', 'type', self.type, 'con-name']
        # format for creating team-subordinate interface
        if self.conn_name is not None:
            cmd.append(self.conn_name)
        elif self.ifname is not None:
            cmd.append(self.ifname)
    ...
    def modify_connection_team_subordinate(self):
        cmd = [self.nmcli_bin, 'con', 'mod', self.conn_name, 'connection.main', self.main]
        # format for modifying team-subordinate interface
    ...
    def create_connection_bond_subordinate(self):
        cmd = [self.nmcli_bin, 'connection', 'add', 'type', 'bond-subordinate', 'con-name']
        # format for creating bond-subordinate interface
        if self.conn_name is not None:
            cmd.append(self.conn_name)
        elif self.ifname is not None:
            cmd.append(self.ifname)
    ...
    def modify_connection_bond_subordinate(self):
        cmd = [self.nmcli_bin, 'con', 'mod', self.conn_name, 'connection.main', self.main]
        # format for modifying bond-subordinate interface
        return cmd
    ...
    def create_connection_bridge_subordinate(self):
        # format for creating bond-subordinate interface
        cmd = [self.nmcli_bin, 'con', 'add', 'type', 'bridge-subordinate', 'con-name']
        if self.conn_name is not None:
            cmd.append(self.conn_name)
        elif self.ifname is not None:
            cmd.append(self.ifname)
        ...
        options = {
            ...
            'bridge-port.priority': self.subordinatepriority,
        }
    ...
    def modify_connection_bridge_subordinate(self):
        # format for modifying bond-subordinate interface
        cmd = [self.nmcli_bin, 'con', 'mod', self.conn_name]
        options = {
            'main': self.main,
    ...
    def create_connection(self):
        ...
        if self.type == 'team':
            if (self.dns4 is not None) or (self.dns6 is not None):
                ...
                cmd = self.create_connection_team()
        elif self.type == 'team-subordinate':
            if self.mtu is not None:
                cmd = self.create_connection_team_subordinate()
                self.execute_command(cmd)
                cmd = self.modify_connection_team_subordinate()
                return self.execute_command(cmd)
            else:
                cmd = self.create_connection_team_subordinate()
        elif self.type == 'bond':
            if (self.mtu is not None) or (self.dns4 is not None) or (self.dns6 is not None):
                ...
                cmd = self.create_connection_bond()
        elif self.type == 'bond-subordinate':
            cmd = self.create_connection_bond_subordinate()
        ...
        elif self.type == 'bridge-subordinate':
            cmd = self.create_connection_bridge_subordinate()
        elif self.type == 'vlan':
    ...
    def modify_connection(self):
        ...
        if self.type == 'team':
            ...
            cmd = self.modify_connection_bond_subordinate()
        elif self.type == 'ethernet':
            cmd = self.modify_connection_ethernet()
        elif self.type == 'bridge':
            cmd = self.modify_connection_bridge()
        elif self.type == 'bridge-subordinate':
            cmd = self.modify_connection_bridge_subordinate()
        elif self.type == 'vlan':
...
def main():
    ...
    module = AnsibleModule(
        argument_spec=dict(
            ...
            subordinatepriority=dict(type='int', default=32),
    ...
    # team-subordinate checks
    if nmcli.type == 'team-subordinate' and nmcli.main is None:
        nmcli.module.fail_json(msg="Please specify a name for the main")
    if nmcli.type == 'team-subordinate' and nmcli.ifname is None:
